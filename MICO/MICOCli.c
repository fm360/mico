...
static void ps_Command(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
 int arv;
  ...
  if (argc == 1) {
    cmd_printf("Usage: ps on/off. WIFI PS mode\r\n");
    return;
  }
  ...
  } else if (!strcasecmp(argv[1], "rfoff")) {
	micoWlanPowerOff();
  } else if (!strcasecmp(argv[1], "mcu")) {
	MicoMcuPowerSaveConfig(1);
	MicoSysLed(0);
  } else if (!strcasecmp(argv[1], "deep")) {
    arv = strtoul(argv[2], NULL, 0);
	printf("deep sleep for %d seconds\r\n", arv);
	msleep(10);
    platform_mcu_enter_standby(arv);
  } 
}
