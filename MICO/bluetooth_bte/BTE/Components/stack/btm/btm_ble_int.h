...
typedef struct
{
    BD_ADDR         resolve_q_random_pseudo[BTM_LE_RESOLVING_LIST_MAX];
    UINT8           resolve_q_action[BTM_LE_RESOLVING_LIST_MAX];
    UINT8           q_next;
    UINT8           q_pending;
} tBTM_BLE_RESOLVE_Q;
...
typedef struct
{
    ...
    *****************************************************/
    ...
    BOOLEAN             enabled;    /* resolving list enabled or not */
