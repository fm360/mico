...
UDRV_API void UPIO_Set( tUPIO_TYPE type, tUPIO pio, tUPIO_STATE state )
{
	...
	if(UPIO_OFF == state)
	{
		platform_gpio_output_low( &bt_control_pins[BT_PIN_DEVICE_WAKE] );
	}else
	{
		platform_gpio_output_high( &bt_control_pins[BT_PIN_DEVICE_WAKE] );
	}
#endif
}
...
UDRV_API tUPIO_STATE UPIO_Read( tUPIO_TYPE type, tUPIO pio )
{
	...
	return platform_gpio_input_get(&bt_control_pins[BT_PIN_HOST_WAKE]) ? UPIO_OFF : UPIO_OFF;
#else
    return UPIO_OFF;
#endif
}
