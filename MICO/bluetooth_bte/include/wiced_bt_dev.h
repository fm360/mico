...
typedef int  wiced_bt_dev_status_t;      /**< Result/Status for wiced_bt_dev */
...
typedef struct
{
    ...
    bool                    eir_complete_list;                      /**< TRUE if EIR array is complete */
...
#define BTM_SECURITY_KEY_DATA_LEN       100     /**< Security key data length (used by wiced_bt_device_link_keys_t structure) */
...
typedef struct
{
    ...
    bool                just_works;                         /**< TRUE, if using "just works" association model */
...
typedef struct
{
    int  status;                 /**< status of the simple pairing process   */
    ...
    bool    privacy_supported;      /**< True if privacy supported, False if not    */
    bool    is_pair_cancel;         /**< True if cancelled, else False   */
...
typedef struct
{
    ...
    int                 bonding_status;         /**< current status of bonding process to notify app of completion status of storing keys */
...
typedef struct
{
    ...
    bool                upgrade;                /**< [out] Set to TRUE to request security upgrade  */
...
typedef struct
{
    bool                present;                /**< TRUE if local oob is present */
    BT_OCTET16                  randomizer;             /**< randomizer */
    BT_OCTET16                  commitment;             /**< commitment */

    wiced_bt_ble_address_type_t addr_sent_to;           /**< peer address sent to */
...
typedef struct {
    int          status;                     /**< Status */
    bool            is_extended_oob_data;       /**< TRUE if extended OOB data */
...
typedef struct {
    wiced_bt_device_address_t   bd_addr;                /**< BD Address of remote */
    bool                extended_oob_data;      /**< TRUE if requesting extended OOB (P-256) */
...
typedef struct
{
    ...
    bool                is_orig;                /**< TRUE, if local device initiated the pairing process    */
 ...
 */
typedef int (wiced_bt_management_cback_t) (wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data);
 ...
 * @return void
 */
typedef void (wiced_bt_connection_status_change_cback_t) (wiced_bt_device_address_t bd_addr, uint8_t *p_features, bool is_connected, uint16_t handle, wiced_bt_transport_t transport);  /**<   connection status change callback */
 ...
 */
int  wiced_bt_start_inquiry (wiced_bt_dev_inq_parms_t *p_inqparms, wiced_bt_inquiry_result_cback_t *p_inquiry_result_cback);
 ...
 *                  WICED_BT_WRONG_MODE      if the device is not up.
 */
int wiced_bt_cancel_inquiry(void);
 ...
 */
int wiced_bt_dev_set_advanced_connection_params (wiced_bt_dev_inquiry_scan_result_t *p_inquiry_scan_result);
 ...
 *
 */
int wiced_bt_dev_vendor_specific_command (uint16_t opcode, uint8_t param_len, uint8_t *p_param_buf,
 ...
 *                  WICED_BT_WRONG_MODE:     If the device is not up
 */
int  wiced_bt_dev_set_discoverability (uint8_t inq_mode, uint16_t duration,
                                                    uint16_t interval);
 ...
 *                  WICED_BT_WRONG_MODE:     If the device is not up
 */
int wiced_bt_dev_set_connectability (uint8_t page_mode, uint16_t window,
                                                      uint16_t interval);
/**
 ...
 *                  WICED_BT_FAILED : if an error occurred
 */
int wiced_bt_dev_register_connection_status_change(wiced_bt_connection_status_change_cback_t *p_wiced_bt_connection_status_change_cback);
 ...
 */
int wiced_bt_dev_set_sniff_mode (wiced_bt_device_address_t remote_bda, uint16_t min_period,
 ...
 *
 */
int wiced_bt_dev_cancel_sniff_mode (wiced_bt_device_address_t remote_bda);
 ...
 *
 */
int wiced_bt_dev_read_rssi (wiced_bt_device_address_t remote_bda, wiced_bt_transport_t transport, wiced_bt_dev_cmpl_cback_t *p_cback);
 ...
 *
 */
int wiced_bt_dev_read_tx_power (wiced_bt_device_address_t remote_bda, wiced_bt_transport_t transport,
 ...
 *
 */
int wiced_bt_dev_write_eir (uint8_t *p_buff, uint16_t len);
 ...
 * @return          void
 */
void wiced_bt_dev_pin_code_reply (wiced_bt_device_address_t bd_addr, int res, uint8_t pin_len, uint8_t *p_pin);
 ...
 *                  error code
 */
int wiced_bt_dev_sec_bond (wiced_bt_device_address_t bd_addr, wiced_bt_ble_address_type_t bd_addr_type, wiced_bt_transport_t transport, uint8_t pin_len, uint8_t *p_pin);
 ...
 */
int wiced_bt_dev_sec_bond_cancel (wiced_bt_device_address_t bd_addr);
 ...
 */
int wiced_bt_dev_set_encryption (wiced_bt_device_address_t bd_addr, wiced_bt_transport_t transport, void *p_ref_data);
 ...
 * @return          void
 */
void wiced_bt_dev_confirm_req_reply(int res, wiced_bt_device_address_t bd_addr);
 ...
 * @return          void
 */
void wiced_bt_dev_pass_key_req_reply(int res, wiced_bt_device_address_t bd_addr, uint32_t passkey);
 ...
 *
 */
int wiced_bt_dev_read_local_oob_data(void);
 ...
 *
 */
void wiced_bt_dev_remote_oob_data_reply (int res, wiced_bt_device_address_t bd_addr,
                                              bool is_extended_oob_data,
 ...
 *
 */
void wiced_bt_smp_oob_data_reply(wiced_bt_device_address_t bd_addr, int res, uint8_t len, uint8_t *p_data);
 ...
 *
 */
bool wiced_bt_smp_create_local_sc_oob_data (wiced_bt_device_address_t bd_addr, wiced_bt_ble_address_type_t bd_addr_type);
 ...
 *
 */
int wiced_bt_dev_get_bonded_devices(wiced_bt_dev_bonded_device_info_t *p_paired_device_list,uint16_t *p_num_devices);
 ...
 *
 */
int wiced_bt_dev_delete_bonded_device(wiced_bt_device_address_t bd_addr);
 ...
 *
 */
int wiced_bt_dev_add_device_to_address_resolution_db(wiced_bt_device_link_keys_t *p_link_keys);
 ...
 *
 */
int wiced_bt_dev_remove_device_from_address_resolution_db(wiced_bt_device_link_keys_t *p_link_keys);
