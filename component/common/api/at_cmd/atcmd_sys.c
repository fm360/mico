...
extern char *mxchip_gpio_test(void);
void fATSGM(void *arg)
{	
	char *result;

	AT_DBG_MSG(AT_FLAG_GPIO, AT_DBG_ALWAYS, "[ATSM]: _AT_SYSTEM_GPIO_TEST_MXCHIP_");
	result = mxchip_gpio_test();
	AT_DBG_MSG(AT_FLAG_GPIO, AT_DBG_ALWAYS, "[ATGM]: %s", result);
}
...
void fATS0(void * arg)
{
	...
	if(argc == 2)
	{
		...
		if(len&0x01== 1 ||len ==0 ||len > MAX_OPT_LENGTH*2 ){
			AT_PRINTK("Length is Invaild, Max length is 32 Byte!");
			return;
		}
		...
		while(temp[i]!='\0')
		{
			if(!(((temp[i]>='0')&&(temp[i]<='9'))||
				...
				AT_PRINTK("Written Data must be HEX!");				
				return;
			}
			i++;
		}
		...
		{
			setdata[i] = ((char2num(temp[j]) << 4) |char2num(temp[j+1]));
		}
	}else{
		AT_DBG_MSG(AT_FLAG_DUMP, AT_DBG_ALWAYS," Format is Error!");
		AT_DBG_MSG(AT_FLAG_DUMP, AT_DBG_ALWAYS," Usage: for example  write data is 0x12,0xab,0x0,,0xef");
		AT_DBG_MSG(AT_FLAG_DUMP, AT_DBG_ALWAYS," ATS0=12ab00ef");		
		return;	
	}

	for (i = 0; i < MAX_OPT_LENGTH - len/2; i++)
		setdata[len/2 + i] = 0xff;

	 efuse_otp_write(0, MAX_OPT_LENGTH, setdata);
...
void fATS1(void *arg)
{
  	u8 content[MAX_OPT_LENGTH] = {0};
        u8 i;
  	efuse_otp_read(0, MAX_OPT_LENGTH, content);

/*8888if efuse never write before, show the content[] = {0}
	for(i = 0; i < 32; i++)
	{
		if(content[i]!= 0xff)
			break;
...
log_item_t at_sys_items[] = {
	...
	{"ATSM", fATSGM,},	// MP GPIO test for mxchip
	{"ATSC", fATSC,},	// Clear OTA signature
	{"ATSR", fATSR,},	// Recover OTA signature
	{"ATSP", fATSP,},	// MP Power related test
	{"ATS0", fATS0,},//write opt efuse block
	{"ATS1", fATS1,},//read opt efuse block	
#endif
#if defined(configUSE_WAKELOCK_PMU) && (configUSE_WAKELOCK_PMU == 1)
