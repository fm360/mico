/*
	...
	How to use this:
		1.You must include platform_stdlib.h in you source fileã€?
...
static int _vsscanf_patch(const char *buf, const char *fmt, va_list args)
{
	...
	while(*fmt) {
		...
		*/
		...
		//åˆ¤æ–­è½¬æ¢çš„å­—ç¬¦ä¸²çš„å®½åº¦æ˜¯å¦å¤§äº?%2d
	...
	for(; *fmt != '\0'; ++fmt) {
		...
		{	/* Length of item is bounded */
			...
			if((*fmt == 'x')||(*fmt == 'X') || (*fmt == 'p') || (*fmt == 'P')) {
				...
				//hback ??¡¥???¨¦?¡­??¡ª??¡ã?????¡ã??????hex_count??¡¥???¨¨????¡ã??????HEX?-¡ª??|??a??¡ã
				while(h_back) {
					hex_count += (h_back & 0xF) ? 1 : 0;
					h_back  = h_back >> 4;
				}

				//¨¨??¨¦??????¡è? example??? ?-¡ª??|?????a????????¡¥?¡±¡§?o?%02x?¡¥?¨¨?¡ä?-¡ª??|¨¨¡é???a?¨C-?????¡­??¦Ì
				if(shift < (hex_count - 1)*4)
					shift = (hex_count - 1)*4;
