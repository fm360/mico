            ...
            if (UartLogCmdChk(UartReceiveData,(UART_LOG_CTL *)pUartLogCtl,1)==2)
            {
                ...
                if (pUartLogCtl->pTmpLogBuf != NULL)
                {
    				...
    				if (pUartLogCtl->TaskRdy)
    					mico_rtos_set_semaphore(&pUartLogCtl->Sema); 
#endif
                }
                else
                {
...
)
{
    ...
    else
    {
        ...
        /* Create a Semaphone */
        mico_rtos_init_semaphore(&(pUartLogCtl->Sema), 32);
        pUartLogCtl->TaskRdy = 0;
#ifdef PLATFORM_FREERTOS
        if (kNoErr != mico_rtos_create_thread(NULL,1, "LOGUART_TASK", RtlConsolTaskRam, 512, NULL))
        {
...
extern mico_semaphore_t	log_rx_interrupt_sema;
#endif
//======================================================
...
)
{
    ...
    do{
#if defined(CONFIG_KERNEL) && !TASK_SCHEDULER_DISABLED
                mico_rtos_get_semaphore(&pUartLogCtl->Sema, MICO_WAIT_FOREVER);
#endif
