...
%tooldir%\objcopy -j "A4 rw" -Obinary %cfgdir%/Exe/application.axf %cfgdir%/Exe/ram_2.bin
if defined %ram3_start (
	%tooldir%\objcopy -j "A6 rw" -Obinary %cfgdir%/Exe/application.axf %cfgdir%/Exe/sdram.bin
)
...
copy %cfgdir%\..\..\bootloader\RTL8711AM\Exe\fw_loader.p.bin %cfgdir%\Exe\fw_loader.p.bin
%tooldir%\padding 32k 0xFF %cfgdir%\Exe\fw_loader.p.bin

:: SDRAM case
if defined %ram3_start (
  copy /b %cfgdir%\Exe\ram_1.p.bin+%cfgdir%\Exe\fw_loader.p.bin+%cfgdir%\Exe\ram_2.p.bin+%cfgdir%\Exe\ram_3.p.bin %cfgdir%\Exe\ram_all.bin	
	copy /b %cfgdir%\Exe\ram_2.ns.bin+%cfgdir%\Exe\ram_3.p.bin %cfgdir%\Exe\ota.bin
	copy /b %cfgdir%\Exe\ram_2.p.bin+%cfgdir%\Exe\ram_3.p.bin %cfgdir%\Exe\mico_app.bin
)

:: NO SDRAM case
if not defined %ram3_start (
  copy /b %cfgdir%\Exe\ram_1.p.bin+%cfgdir%\Exe\fw_loader.p.bin+%cfgdir%\Exe\ram_2.p.bin %cfgdir%\Exe\ram_all.bin	
	copy /b %cfgdir%\Exe\ram_2.ns.bin %cfgdir%\Exe\ota.bin
	copy /b %cfgdir%\Exe\ram_2.p.bin %cfgdir%\Exe\mico_app.bin
